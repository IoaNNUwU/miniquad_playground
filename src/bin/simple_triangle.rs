...
fn myvertex(x: f32, y: f32, z: f32, color: MyVec3) -> MyVertex {
    MyVertex { pos: myvec3(x, y, z), color  }
}
...
fn make_pipelone_and_bindings(ctx: &mut dyn RenderingBackend) -> (Pipeline, Bindings) {
    ...
    #[rustfmt::skip]
    let vertices = [
        myvertex(-0.5, -0.5, 0.0, RED),
        myvertex( 0.5,  0.5, 0.0, GREEN),
        myvertex( 0.5, -0.5, 0.0, BLUE),
    ];
    ...
    let pipeline = ctx.new_pipeline(
        &[BufferLayout { //    POSITION \/   \/ COLOR       
            stride: (size_of::<f32>() * (3 + 3)) as i32,
